...
import detectron.modeling.cluster_rcnn_heads as cluster_rcnn_heads
...
def generalized_rcnn(model):
    ...
    return build_generic_detection_model(
        ...
        add_cluster_roi_box_head_func=get_func(cfg.Cluster_RCNN.ROI_BOX_HEAD)
    )
...
def build_generic_detection_model(
    ...
    add_cluster_roi_box_head_func=None
):
    def _single_gpu_build_func(model):
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            if cfg.MODEL.Cluster_RCNN_ON:
                # Add the Cluster R-CNN head
                head_loss_gradients['cluster_box'] = _add_cluster_rcnn_head(
                    model, add_cluster_roi_box_head_func, blob_conv, dim_conv,
                    spatial_scale_conv
                )
...
def _add_cluster_rcnn_head(
    model, add_cluster_roi_box_head_func, blob_in, dim_in, spatial_scale_in
):
    """Add a cluster R-CNN head to the model."""
    cluster_rcnn_heads.add_cluster_proposals(model)
    blob_frcn, dim_frcn = add_cluster_roi_box_head_func(
        model, blob_in, dim_in, spatial_scale_in
    )
    cluster_rcnn_heads.add_cluster_rcnn_outputs(model, blob_frcn, dim_frcn)
    if model.train:
        loss_gradients = cluster_rcnn_heads.add_cluster_rcnn_losses(model)
    else:
        loss_gradients = None
    return loss_gradients
